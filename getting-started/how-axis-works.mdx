...
```
User submits SwapRequest
    ↓
Order queued in BatchQueue PDA
    ↓
Window closes (every 10 slots)
    ↓
Searchers compete via Jito bundle to call ClearBatch
    ↓
Winning searcher clears batch at oracle-bounded price
    ↓
User calls Claim to receive output tokens
```
